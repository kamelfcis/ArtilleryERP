...
router.post('/', requireAuth, requireAnyRole(...WRITE_ROLES), async (req, res, next) => {
  try {
    const g = pickFields(req.body ?? {}, GUEST_FIELDS)
...
router.patch('/:id', requireAuth, requireAnyRole(...WRITE_ROLES), async (req, res, next) => {
  try {
    const body = pickFields(req.body ?? {}, GUEST_FIELDS)
    const built = buildUpdateSet(body, 1, GUEST_FIELDS)
    if (!built) {
...
router.delete('/:id', requireAuth, requireAnyRole(...WRITE_ROLES), async (req, res, next) => {
  try {
    const { rows: before } = await pool.query(`SELECT id, phone, first_name FROM guests WHERE id = $1`, [
      req.params.id,
    ])
